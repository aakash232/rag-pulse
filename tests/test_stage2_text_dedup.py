    ...
    chunks = [
        {
            "id": "a",
            "text": text,
            "embedding": _rand_unit(DIM, 0),
            "metadata": {"created_at": "2024-01-01T00:00:00Z"},
        },
        {
            "id": "b",
            "text": text + " and night",  # minimal change, high Jaccard
            "embedding": _rand_unit(DIM, 99),  # orthogonal to a
            "metadata": {"created_at": "2024-02-01T00:00:00Z"},
        },
        {
            "id": "c",
            "text": "Completely unrelated content about databases and SQL queries",
...
def test_text_dedup_adds_channel_to_existing_embedding_group(tmp_path):
    ...
    chunks = [
        {
            "id": "a",
            "text": text,
            "embedding": (base / np.linalg.norm(base)).tolist(),
            "metadata": {"created_at": "2024-01-01T00:00:00Z"},
        },
        {
            "id": "b",
    ...
    channels_before = json.loads(conn.execute("SELECT detection_channels FROM dedup_groups").fetchone()[0])
    ...
    channels_after = json.loads(conn.execute("SELECT detection_channels FROM dedup_groups").fetchone()[0])
    ...
    chunks = [
        {
            "id": "old",
            "text": text,
            "embedding": _rand_unit(DIM, 10),
            "metadata": {"created_at": "2022-01-01T00:00:00Z"},
        },
        {
            "id": "new",
            "text": text + " of the year",
    ...
    chunks = [
        {
            "id": "a",
            "text": text,
            "embedding": _rand_unit(DIM, 20),
            "metadata": {"created_at": "2024-01-01T00:00:00Z"},
        },
        {
            "id": "b",
            "text": text + " and night and morning",
...
def test_text_dedup_merges_two_different_groups(tmp_path):
    ...
    chunks = [
        # Group A: a1 + a2 (embedding near-dups)
        {
            "id": "a1",
            "text": long_text,
            "embedding": _rand_unit(DIM, 30),
            "metadata": {"created_at": "2024-01-01T00:00:00Z"},
        },
        {
            "id": "a2",
            "text": long_text + " indeed",
            "embedding": _rand_unit(DIM, 31),
            "metadata": {"created_at": "2024-01-02T00:00:00Z"},
        },
        # Group B: b1 + b2 (embedding near-dups)
        {
            "id": "b1",
            "text": long_text + " and night",
            "embedding": _rand_unit(DIM, 40),
            "metadata": {"created_at": "2024-02-01T00:00:00Z"},
        },
        {
            "id": "b2",
            "text": long_text + " and night too",
            "embedding": _rand_unit(DIM, 41),
    ...
    channels = set(json.loads(conn.execute("SELECT detection_channels FROM dedup_groups").fetchone()[0]))
